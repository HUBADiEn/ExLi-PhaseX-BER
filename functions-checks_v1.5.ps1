...
.NOTES
    File:           functions-checks_v1.5.ps1
    Version:        1.5
    Änderungshistorie:
        1.5 - Check 7 (Release-Datum): Vergleich auf 2-stelliges Jahr
            - Jahr wird auf letzte 2 Ziffern reduziert (2030 → 30, 1930 → 30)
            - PMS "06.09.30" und PIM "20300906" sind identisch
        1.4 - Check 14 (L-Prio): Fix Korrelationspruefung
            - Prueft jetzt auf 'PrioEP Diff' Spalte (nicht auf Check 10 Ergebnis)
            - Funktioniert auch wenn Check 10 = "ok - Diff von X" (Toleranz erfuellt)
...
# =====================================================================
$script:ModuleVersion_Checks = "1.5"
...
function Invoke-Check7_ReleaseDatum {
    ...
    # V1.5: Vergleiche nur Tag, Monat und 2-stelliges Jahr (letzte 2 Ziffern)
    # Grund: PMS speichert 2-stellig (06.09.30), PIM 4-stellig (20300906)
    # Beide sollen als 30 verglichen werden
    ...
    # Bei Fehler: Zeige beide Daten mit 2-stelligem Jahr
...
function Invoke-Check14_LPrio {
    ...
    # V1.4: Pruefe Korrelation mit PrioEP-Diff
    # Prueft auf 'PrioEP Diff' Spalte (nicht auf Check 10 Ergebnis!)
    # So funktioniert es auch wenn Check 10 = "ok - Diff von X" (Toleranz erfuellt)
    $prioEPDiffValue = $Dataset.'PrioEP Diff'
    if (-not [string]::IsNullOrEmpty($prioEPDiffValue) -and $prioEPDiffValue -ne '' -and $prioEPDiffValue -ne '---') {
        # Es gibt eine PrioEP Differenz
        $prioEPDiff = [decimal]0
        $prioEPDiffParsed = [decimal]::TryParse($prioEPDiffValue.ToString(), [ref]$prioEPDiff)
        ...
        if ($prioEPDiffParsed -and $prioEPDiff -ne 0) {
            ...
            if ($pmsLPrioParsed -and $pimLPrioParsed) {
                ...
                # Wenn L-Prio-Diff < 250 * PrioEP-Diff -> Warnung
