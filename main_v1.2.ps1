...
# =====================================================================
$script:ModuleVersion_Main = "1.2"

# HINWEIS: $script:ScriptVersion wird von Start.ps1 gesetzt (z.B. "Berechnung_V1.6")
...
function Pause-Ende {
    ...
    Write-Host "Vergleich Phase X - Berechnung (Script Version $($script:ScriptVersion))" -ForegroundColor White
    Write-Host ""
...
function Invoke-MainLogic {
    try {
        ...
        Write-Host "--- Skript-Version $($script:ScriptVersion) ---`n" -ForegroundColor Gray
        ...
        if ($script:UseExcelExport) {
            ...
            Apply-SummaryRow -Path $OutputFilePath_All -WorksheetName "Vergleich" -HeaderSummary $headerSummary -WarningSummary $warningSummary -ScriptVersion $script:ScriptVersion -SupplierNumber $script:pmsSupplier
            ...
            if ($exportErrors.Count -gt 0) {
                ...
                Apply-SummaryRow -Path $OutputFilePath_Errors -WorksheetName "Fehler" -HeaderSummary $headerSummary -WarningSummary $warningSummary -ScriptVersion $script:ScriptVersion -SupplierNumber $script:pmsSupplier
